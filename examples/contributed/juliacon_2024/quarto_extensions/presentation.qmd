...
Ronny (!!! FIX: Keep that here? Otherwise remove !!!)
...
Within some Julia package we aim to

- write tutorials (`.qmd`) in `tutorials/`
- run them when generating the documentation
- should render them into `docs/src/tutorials/` (as `.md`)

### An Example

(!!! Still needs to be fixed, is executed but should not!!!)
...
Quarto notebooks allow for an easy way to have

- executed
- reproducible
- cached
- inegrated (with `@ref` links)
- citing (with `[citekey](@cite)` and `DocumenterCitations.jl`)

tutorials within your documentation.
...
- the [QuartoNotebookRunner.jl](https://github.com/PumasAI/QuartoNotebookRunner.jl)
  should provide native Quarto rendering in pure Julia
